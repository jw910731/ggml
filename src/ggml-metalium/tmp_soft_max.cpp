...
soft_max_device::SoftMaxDeviceOperation::program_factory_t
soft_max_device::SoftMaxDeviceOperation::select_program_factory(
    const operation_attributes_t& /*attrs*/,
    const tensor_args_t& /*tensor_args*/)
{
    return program::SoftMaxProgramFactory{};
}

soft_max_device::SoftMaxDeviceOperation::spec_return_value_t
soft_max_device::SoftMaxDeviceOperation::compute_output_specs(
...
soft_max_device::SoftMaxDeviceOperation::tensor_return_value_t
soft_max_device::SoftMaxDeviceOperation::create_output_tensors(
    const operation_attributes_t& operation_attributes,
    const tensor_args_t& tensor_args)
{
...
void soft_max_device::SoftMaxDeviceOperation::validate_on_program_cache_miss(
    const operation_attributes_t& /*operation_attributes*/,
    const tensor_args_t& tensor_args)
{
    ...
    if (tensor_args.mask.has_value()) {
        ...
        TT_FATAL(
            a_tensor.logical_shape()[0] % mask_tensor.logical_shape()[0] == 0 &&
            a_tensor.logical_shape()[1] % mask_tensor.logical_shape()[1] == 0 &&
            a_tensor.logical_shape()[2] == mask_tensor.logical_shape()[2] &&
            a_tensor.logical_shape()[3] == mask_tensor.logical_shape()[3],
            "Mask shape must be broadcastable in the first 2 dimensions and same to input in the last 2");
    }
}

void soft_max_device::SoftMaxDeviceOperation::validate_on_program_cache_hit(
    const operation_attributes_t& operation_attributes,
    const tensor_args_t& tensor_args)
{
...
std::tuple<soft_max_device::operation_attributes_t, soft_max_device::tensor_args_t>
soft_max_device::SoftMaxDeviceOperation::invoke(
    const Tensor& input,
    ...
    float scale)
{
    return {
        ...
        tensor_args_t{input, std::move(mask)}
    };
}

// ---- SoftMaxProgramFactory ----

soft_max_device::program::SoftMaxProgramFactory::cached_program_t
soft_max_device::program::SoftMaxProgramFactory::create(
    const operation_attributes_t& operation_attributes,
    const tensor_args_t& tensor_args,
        ...
        for (const auto& range : group.ranges()) {
            for (const auto& core : range) {
                ...
                SetRuntimeArgs(program, reader, core, std::vector<uint32_t>{
                    a->address(), width_tiles, height_tiles, n_head, batch,
                    (uint32_t)mask_addr, mask_n_head, mask_batch});
                SetRuntimeArgs(program, compute, core, std::vector<uint32_t>{
                    width, height, batch, n_head, batch});
                SetRuntimeArgs(program, writer, core, std::vector<uint32_t>{
                    o->address(), width_tiles, height_tiles, n_head, batch});
            }
        }
    }
...
void soft_max_device::program::SoftMaxProgramFactory::override_runtime_arguments(
